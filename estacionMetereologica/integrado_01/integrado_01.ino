 ...
 * Circuito: Solo se necesita conectar una resistencia de 10K @ 5V en la salida de la VELETA (pin4 de roseta) y un condensador (1e-6 o 100e-9F) a TIERRA (v�ase datasheet).
 *				Toda la otra parte de la circuiter�a, es conexi�n a los puertos digitales/analogos de acuerdo a lo indicado en el programa.
 */
...
const byte LUMENS_ADC = A1;
const byte UV_ADC = A2;
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
void calcWeather()
{
	...
	//si se desea el cardinal de la direccion se utilizar�a la funcion
	//nameDirWind[]
...
//VMH: Se reemplaza unidad m�trica de acumulaci�n
float get_wind_speed()
{
	...
	float windSpeed = (float)windClicks / deltaTime;	//se obtiene la cantidad de veces que se activ� el interruptor magn�tico (1 interrupci�n es igual a 2.4kph)
	windClicks = 0;										//se vuelve a inicializar la variable volatil
	lastWindCheck = millis();							//se registra la actual medici�n de tiempo en milisegundos
...
int get_wind_direction()
{
	...
	// VMH: se realiza el cambio de parametro original que correspond�a al valor en grados por
...
void printSensors()
{
	...
	Serial.println(uv());
	delay(1);
	Serial.println(winddir);					//lectura DIRECCION (Grados) instant�nea
	delay(1);
	Serial.println(_nameWDir);					//lectura DIRECCION (Cardinal) instant�nea
	delay(1);
	...
	delay(1);
	Serial.println(windgust_kph, 2);			//lectura RAFAGA instant�nea
	delay(1);
	...
	delay(1);
	Serial.println(dailyrainin, 2);			//lectura instantanea de la lluvia acumulada (habr�a que considerar resetear el valor desde el acumulador)
	delay(1);
...
void loop()
{
    ...
    lastSecond += 1000;
        ...
        //Originalmente la pregunta de los par�metros iba ac�, pero como ya no se necesita
		//preguntar a cada segundo por lo par�metros, sino m�s bien, por cada consulta
...
//lectura lumens
	...
	conv = lect*0.0048828125;
	lux=((2500/conv)-500)/10;
	return(lux);
}
...
{
//lectura uv
	sum=0;		//iniciado por cada llamada

	for(int i=0;i<1024;i++)
	{
		sensorUV=analogRead(UV_ADC);
		sum=sensorUV+sum;
		delay(2);
	}

	sum = sum >> 10;			//eliminar valores de menor peso.
	Vsig = sum*4980.0/1023.0;	// Vsig is the value of voltage measured from the SIG pin of the Grove interface
	if (Vsig < 50) {return(0);}
	if (Vsig > 50 && Vsig < 227) {return(1);}
	if (Vsig > 227 && Vsig < 318) {return(2);}
	if (Vsig > 318 && Vsig < 408) {return(3);}
	if (Vsig > 408 && Vsig < 503) {return(4);}
	if (Vsig > 503 && Vsig < 606) {return(5);}
	if (Vsig > 606 && Vsig < 696) {return(6);}
	if (Vsig > 696 && Vsig < 795) {return(7);}
	if (Vsig > 795 && Vsig < 881) {return(8);}
	if (Vsig > 881 && Vsig < 976) {return(9);}
	if (Vsig > 976 && Vsig < 1079) {return(10);}
	if (Vsig > 1079 && Vsig < 1170) {return(11);}
	if (Vsig > 1170) {return('11+');}
}
